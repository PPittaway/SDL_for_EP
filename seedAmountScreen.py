...
class seedAmountScreener(QtWidgets.QWidget):
    def __init__(self, parent, main):
        super(seedAmountScreener, self).__init__(parent)
    ...
    ### Function takes user inputs and calculates required conditions to complete the sequence of experiments ###
        ...
        R_sMax = (w_s*(w_eMax - w_f))/(w_f*(w_eMax - w_s))
        self.R_seq = np.round(np.linspace(R_sMin, R_sMax, numExp), 4)

        self.v_seed_seq = np.array([])
        self.v_emulsion_seq = np.array([])
        self.v_total_seq = np.array([])
        self.w_emulsion_seq = np.array([])

        for r in self.R_seq:
            emulsionFeedsIndex = np.linspace(1, self.nEmulsionFeeds, self.nEmulsionFeeds)
            summation = 0
            for i in emulsionFeedsIndex:
                summation_temp = 1/(1 + ((S_density/E_density)*((w_s/(w_f*r)) - 1))*(1-((self.nEmulsionFeeds-i)/self.nEmulsionFeeds)))
                summation = summation + summation_temp
            A = (S_density/E_density)*((w_s/(w_f*r)) - 1)
            B = (nCSTRs - self.nEmulsionFeeds)/(1 + A)
            v_seed = (self.V_CSTR/self.tau)*(summation + B)
            self.v_seed_seq = np.append(self.v_seed_seq, v_seed)
            v_emulsion = ((S_density*v_seed)/E_density)*((w_s/(w_f*r)) - 1)
            ...
            self.w_emulsion = (w_f*w_s*(1-r))/(w_s - w_f*r)
            self.w_emulsion_seq = np.append(self.w_emulsion_seq, self.w_emulsion)

        self.v_monomer_seq = np.array([])
        self.v_Aq_seq = np.array([])

        for i, r in zip(self.v_emulsion_seq, self.w_emulsion_seq):
            v_monomer = (E_density*i*(r - w_Aq))/(M_density*(1 - w_Aq))
            self.v_monomer_seq = np.append(self.v_monomer_seq, v_monomer)

            v_Aq = (E_density*i*(1 - r))/(Aq_density*(1 - w_Aq))
        ...
        V_waste = Vol_seed_waste + Vol_monomer_waste + Vol_Aq_waste + 3.5*numExp*self.cleanRate*self.cleanTime + self.cleanTime*self.cleanRate + flushTime*self.cleanRate + 1*self.cleanRate
        ...
        for i in self.expIndex:
            ...
            R_seed = QtWidgets.QTableWidgetItem(str(np.round(self.R_seq[i], 4)))
            w_emulsion = QtWidgets.QTableWidgetItem(str(np.round(self.w_emulsion_seq[i], 4)))
