...
class TC08USB(object):
    def __init__(self, dll_path="C:\Program Files\Pico Technology\PicoLog 6"):
        dll_filename = os.path.join(dll_path, 'usbtc08.dll')
        try: 
            self._dll = ctypes.windll.LoadLibrary(dll_filename)
        except:
            print("Could not find dynamic library 'usbtc08.dll' on the specified path")
    ...
    def get_single(self):
        return(self._dll.usb_tc08_get_single(self._handle, ctypes.c_int64(self._temp.ctypes.data), ctypes.c_int64(self._overflow_flags.ctypes.data), ctypes.c_int64(self._units.value)))
